...
describe('SlideshowViewer Component', () => {
  ...
  beforeEach(() => {
    ...
    vi.mocked(slideshowApi.getSlideshowDetail).mockResolvedValue(mockSlideshowDetail);
  });
  ...
  describe('Error Handling', () => {
    ...
    it('shows go back button on error when onExit is provided', async () => {
      ...
      renderWithProviders(<SlideshowViewer slideshowId={1} onExit={handleExit} />);
  ...
  describe('Exit Functionality', () => {
    it('shows exit button when onExit is provided', async () => {
      ...
      await waitFor(() => {
        expect(screen.getByLabelText(/exit presentation/i)).toBeInTheDocument();
      });
    });
    ...
    it('does not show exit button when onExit is not provided', async () => {
      ...
      expect(screen.queryByLabelText(/exit presentation/i)).not.toBeInTheDocument();
    });
  });

  describe('Fullscreen', () => {
    it('shows fullscreen button when allowFullscreen is true', async () => {
      renderWithProviders(<SlideshowViewer slideshowId={1} allowFullscreen={true} />);
    ...
    it('does not show fullscreen button when allowFullscreen is false', async () => {
      renderWithProviders(<SlideshowViewer slideshowId={1} allowFullscreen={false} />);
